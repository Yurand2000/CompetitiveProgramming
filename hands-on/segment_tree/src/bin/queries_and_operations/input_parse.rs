...
pub fn read_input<'a, T>(input: &mut T) -> (Vec<i32>, Vec<Operation>, Vec<Query>)
...
fn read_size_from_io<'a, T>(input: &mut T) -> (usize, usize, usize)
    where T: Iterator<Item = String>
{
    let sizes_str = input.next().unwrap();

    let mut sizes_str = sizes_str.trim().split_whitespace();
    ...
    where T: Iterator<Item = String>
{
    ...
    let mut array_str = array_str.trim().split_whitespace();
...
fn read_ops_from_io<'a, T>(input: &mut T, ops: usize) -> Vec<Operation>
    where T: Iterator<Item = String>
{
    ...
    for _ in 0..ops
    {
        ...
        let mut ops_reader = ops_str.trim().split_whitespace();

        ops_vec.push(Operation(
            ops_reader.next().unwrap().parse().unwrap(),
            ops_reader.next().unwrap().parse().unwrap(),
            ops_reader.next().unwrap().parse().unwrap()
        ));
    }
    ...
    where T: Iterator<Item = String>
{
    ...
    for _ in 0..queries
    {
        ...
        let mut query_reader = query_str.trim().split_whitespace();
